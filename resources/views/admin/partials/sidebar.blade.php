...
<aside id="layout-menu" class="layout-menu menu-vertical menu bg-menu-theme">
    ...
    <ul class="menu-inner">
        @foreach (getAccessibleMenus() as $menu)
            @php
                // Cek apakah menu saat ini aktif (baik menu utama atau salah satu anaknya)
                $activeChild = $menu->children
                    ->filter(function ($child) {
                        return request()->is(trim($child->url, '/'));
                    })
                    ->isNotEmpty();

                $isActive = request()->is(trim($menu->url, '/')) || $activeChild ? 'active' : '';
                $hasChildren = $menu->children->count() > 0;
            @endphp

            <li class="menu-item {{ $isActive }} {{ $hasChildren && $activeChild ? 'open' : '' }}">
                <a href="{{ $hasChildren ? '#menu-' . $menu->id : url($menu->url) }}"
                    class="menu-link {{ $hasChildren ? 'menu-toggle' : '' }} {{ !$hasChildren ? $isActive : '' }}"
                    @if ($hasChildren) role="button"
                    aria-expanded="{{ $activeChild ? 'true' : 'false' }}"
                    aria-controls="menu-{{ $menu->id }}" @endif>
                    <i class="menu-icon tf-icons bx bx-{{ $menu->icon }}"></i>
                    <div data-i18n="{{ $menu->name }}">{{ $menu->name }}</div>
                </a>

                @if ($hasChildren)
                    <ul class="menu-sub collapse {{ $activeChild ? 'show' : '' }}" id="menu-{{ $menu->id }}">
