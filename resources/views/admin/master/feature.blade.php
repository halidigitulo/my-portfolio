<div class="row my-3">
    <div class="col">
        @can('feature.create')
            <button class="btn btn-primary mb-3" id="btn-add-feature"><i class="ri-add-line"></i> Add Data</button>
        @endcan
        <div class="table-responsive-md">
            <table class="table table-sm table-hover table-striped" id="feature-table">
                <thead>
                    <tr>
                        <th width="5%">#</th>
                        <th>Feature</th>
        ...
        <!-- Modal -->
        <div class="modal fade" id="modalfeature" tabindex="-1" aria-hidden="false">
                ...
                <form id="formfeature">
                    <div class="modal-content">
                        ...
                        <div class="modal-body">
                            ...
                            <input type="text" class="form-control mb-2" id="feature_nama"
                                placeholder="Feature" required>
                            <textarea class="form-control mb-2" id="deskripsi" placeholder="Deskripsi"></textarea>
                        </div>
    ...
    <script>
        $(function() {
            ...
            $(document).on('click', '.edit-feature', function() {
                let id = $(this).data('id');
                $.get(`/admin/feature/${id}`, function(data) {
                    $('#modalfeature').modal('show');
                    $('.modal-title').text('Edit Data');
                    $('#feature_id').val(data.id);
                    $('#feature_nama').val(data.nama);
                    $('#deskripsi').val(data.deskripsi);
                    $('#poin').val(data.poin);
                });
            });

            $('#formfeature').submit(function(e) {
                e.preventDefault();
                let id = $('#feature_id').val();
                let url = id ? `/admin/feature/${id}` : `{{ route('feature.store') }}`;
                ...
                $.ajax({
                    ...
                    success: function(res) {
                        ...
                        $('#formfeature')[0].reset(); // Reset form
                    },
                    error: function(err) {
            ...
            $(document).on('click', '.hapus-feature', function() {
                let id = $(this).data('id');
                Swal.fire({
                    title: 'Are you sure?',
        ...
        $('#modalfeature').on('hidden.bs.modal', function() {
            // Arahkan fokus ke tombol pemicu modal
            $('#btnOpenModal').focus();
        });

        $('#modalfeature').on('hide.bs.modal', function() {
